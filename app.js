...
for (let i = 0; i < recipes.length; i++) { // loop through the recipes array
    ...
    <ul>
    ${ingredients.map(ingredient => `<li>${ingredient}</li>`).join("")}// map the ingredients array to a list of ingredients
    </ul>
    </div>`;
    document.body.innerHTML += recipeHTML;// add the recipe to the HTML page
}
